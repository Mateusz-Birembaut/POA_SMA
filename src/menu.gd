...
func _ready() -> void:
	...
	NbSlider.value_changed.connect(setNbStudents);
	SpeedSlider.value_changed.connect(setSpeed);
	CandySlider.value_changed.connect(setCandy);


func launch() -> void:
	GM.number_of_students = NbSlider.value
	GM.initial_cheese = CandySlider.value
	GM.prof_speed = SpeedSlider.value * 1000
